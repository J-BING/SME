...
# Put the dbv data absolute path here
datapath = 'dbv_s/'
...
f = open('../data/dbv_entity2idx.pkl', 'w')
g = open('../data/dbv_idx2entity.pkl', 'w')
...
for datatyp in ['train', 'valid', 'test']:
    ...
    if 'data' not in os.listdir('../'):
        os.mkdir('../data')
    f = open('../data/dbv_s-%s-lhs.pkl' % datatyp, 'w')
    g = open('../data/dbv_s-%s-rhs.pkl' % datatyp, 'w')
    h = open('../data/dbv_s-%s-rel.pkl' % datatyp, 'w')
...
if len(remove_tst_ex) !=0:
    f = open(datapath + 'dbv_s-train.txt', 'a')
